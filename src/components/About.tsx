...
const About = () => {
  const [showHologram, setShowHologram] = createSignal(false);
  const [isActivating, setIsActivating] = createSignal(false);
  let containerRef: HTMLDivElement | undefined;

  const toggleHologram = () => {
    if (showHologram() || isActivating()) return; // Already on or activating
    setIsActivating(true);
    setTimeout(() => {
      setShowHologram(true);
      setIsActivating(false);
    }, 1500); // Activation sequence time
  };
  ...
  return (
    <div class="container-fluid px-0">
      ...
      <div class="hologram-wrapper" ref={containerRef}>
        {/* Projector Base */}
        <div class="projector-base-container">
          <div class={`projector-lens ${isActivating() ? 'lens-active' : ''}`}></div>
          <div class="projector-body">
            <div class="tech-lines"></div>
          </div>
          <div class="projector-glow"></div>
        </div>

        {/* Projection Beam */}
        <div class={`hologram-beam ${showHologram() || isActivating() ? 'beam-visible' : ''}`}></div>

        {/* Holographic Content */}
        <div class={`hologram-content ${showHologram() ? 'visible' : ''}`}>
